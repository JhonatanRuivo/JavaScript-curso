...
var sinal = prompt(
  'Digite um destes sinais +, -, /, * para realizar uma operação...'
) // Sinal para decidir qual operação será realizada

while (sinal != '+' && sinal != '-' && sinal != '*' && sinal != '/') {
  // Validação para realizar uma operação válida por nosso código
...
function soma(a, b) {
  // Função de realizar a operação a partir do caractere escolhido pelo usuário passando os números como parâmetro
  if (sinal == '+') {
