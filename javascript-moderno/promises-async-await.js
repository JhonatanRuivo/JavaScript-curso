...
// A promise é uma função que me retorna uma promessa que pode ser que ela foi resolvida ou foi rejeitada
{
    ...
    async function asyncFunctionExemple() {   // Com a função async teremos dentro dela agora o método await, este método fará com que o restante da 
        await testPromise().then((res)=> {    // da função execute somente depois que a função testPromise() que está no await for executado.
            alert(res)
        }).catch((err)=> {
            alert(err)
        })                   
