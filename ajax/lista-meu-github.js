...
const myList = document.querySelector('.list-repos')                                   // Pegamos nosso element ul no html e adicionamos
                                                                                       // nesta const myList.
const myRequest = new Request(`https://api.github.com/users/jhonatanruivo/repos`)      // Fazemos esta requisição na api do github

fetch(myRequest)                                    
.then((Response) => Response.json())    
.then((data)=>{                                             // Com a resposta em json manipulamos os dados para criar uma lista 
    for(var i = 0; i < data.length; i++) {                  // através de um for para pegar todos os repositórios da api
        const repos = data[i]                               
